...
On constate que le second argument de la fonction est optionnel. Cela veut dire que n'importe quelle entité tapant `/tp <selector>` va se téléporter à la position de l'entité sélectionnée. Signifiant que le sélecteur ne doit renvoyer qu'une seule entité. Ou alors simplement les coordonnées à la place du sélecteur<br>
...
Le dernier lui, plus complexe, va permettre de regarder une entité prise par un sélecteur. L'argument optionnel `facing_anchor` précise si on doit regarder les pieds (feet) ou les yeux (eyes) de l'entité avec comme valeur par défaut: feet.
